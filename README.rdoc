...
The backend uses a customized ActiveAdmin interface as the backend.  The backed supports all the CRUD operations for the various models in the system e.g. students, parents, etc.
...
* PostgresSql
  - Main database
* Redis
  - Used by ActiveJob for queueing background processes
