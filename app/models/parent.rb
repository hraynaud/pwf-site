class Parent < User
  ...
  scope :former, ->{with_confirmed_registrations.where.not(id: Parent.with_current_confirmed_registrations.select("users.id"))}
  ...
  scope :with_training_program, -> { with_current_registrations.merge(StudentRegistration.in_training_program) }
